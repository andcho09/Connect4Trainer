...
public class Board {
	...
	/**
	 * Normalise the board which is useful for creating hashes or persistent storage. This will:
	 * <ol>
	 ...
	 * </ol>
	 *
	 * @return a new normalised board which could look the same as the current board
	 */
	public Board normalise() {
		final Board result;
		final int[] discCount = BoardHelper.countDiscs(this);
		if (discCount[0] < discCount[1]) {
			// there are more red discs than yellow
			result = this.swap();
		} else {
			result = new Board(this);
		}
		for (int i = 0; i < result.nCols / 2; i++) {
			final int left = result.board[i];
			final int right = result.board[nCols - 1 - i];
			if (left < right) {
				result.reverse();
				return result;
			} else if (left > right) {
